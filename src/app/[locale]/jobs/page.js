...
useEffect(()=>{
  ...
  return (
    <>
       ...
       <section className="jobs">
        <div className="con">
          <div className="content">
            <h2> {jobs.length>0 ? jobs.length:0} {t("jobNum")}</h2>
            {
               jobs.length>0? <div className="allJobs">
              {
                jobs.map((job)=>{
                  return(
                    ...
                    <button
                    onClick={(e)=>{e.preventDefault(); setjobID(job.id) }}
      ...
      <ContactUS />
      {
        jobOne?.length>0? <ModalJop lang={params.locale} data={jobOne[0]} /> :null
